...
First fork-line minor release after v1.0.0. Adds task tiering and journey-aware ITC negotiation.

### Inner-Loop Tier Model

`writing-plans` now classifies every task using a 3-tier rubric (`trivial`/`standard`/`heavy`), and `subagent-driven-development` dispatches a different subagent chain per tier. Default-heavy rule plus an implementer escalation path means simple tasks are fast while ambiguous ones still get the full review pipeline.

- New `skills/writing-plans/tier-rubric.md` with clause-cited match rules (`T*`, `S*`, `H*`)
- `writing-plans/SKILL.md` wires tier classification into plan output (every task carries `tier` + `tier_reason`)
- `subagent-driven-development/SKILL.md` adds tier-aware dispatch and an end-of-run ledger
- `implementer-prompt.md` adds the tier-escalation exit path
- `spec-reviewer-prompt.md` and `testing-agent-prompt.md` updated for tier-aware review

### Journey Context in ITC Negotiation

ITC handshakes now exchange explicit user-journey context, and each negotiation round carries knowledge of the previous round so 5-round negotiations converge.

- `coding-agent-prompt.md` and `testing-agent-prompt.md` add Journey Context sections
- `testing-agent-prompt.md` adds a journey-coverage check in the ITC review block
- Journey context resolution protocol added to ITC negotiation
- ITC term explained inline in the skill so agents don't chase definitions

### Onboarding & Install

- `docs/guides/quick-start.md` install instructions clarified for the marketplace flow
- `docs/guides/README.md` adds "When to use Superpowers vs plain Claude Code" decision section
- New `docs/guides/SDLC_mental_model_and_workflow.md` covers the full development lifecycle

### Marketplace

- Version bump to 1.1.0 in `.claude-plugin/marketplace.json`, `.claude-plugin/plugin.json`, `.cursor-plugin/plugin.json`, `package.json`

## v1.0.0 (2026-04-25)

First fork-line release. Introduces the E2E test harness, ITC negotiation, and journey-based coverage matrix. See README "What's New" → v1.0.0 section for the full story; carried forward from squash commit `8455725` on `release`.

### ITC Negotiation and Runtime Test Harness

- Pre-task ITC negotiation between `coding-agent` and `testing-agent` with 5-round cap and BLOCKED handling
- Solution-level ITC for plan-wide coverage matrix
...
- `contracts/` directory for runtime ITC files

### Journey-Based Coverage Matrix

- `brainstorming` adds Journey Enumeration step, produces `<topic>-journeys.yaml`
- `writing-plans` adds `contributes_to` field per task linking to journey IDs
- Solution ITC produces a coverage matrix: every journey × testing strategy must be a runnable scenario or an explicit N/A
- New `skills/subagent-driven-development/testing-strategies.md` defines the 5 strategies (`happy_path`, `negative_path`, `state_persistence`, `feature_interaction`, `auth_boundary`)

### Team Adoption Guides

- New `docs/guides/` directory: `quick-start.md`, `daily-workflows.md`, `skills-cheatsheet.md`, `team-leads.md`, `README.md`
